...
public class LeverScript : XRBaseInteractable
{
    ...
    void UpdateValue()
    {
        ...
        float threshold = 0.1f; // This threshold can be adjusted based on your requirements
        bool nearMinAngle = minAngleDistance < threshold;
        bool nearMaxAngle = maxAngleDistance < threshold;
        ...
        var newValue = m_Value; // keep the current value as the default

        if (nearMaxAngle)
        {
            newValue = true;
        }
        else if (nearMinAngle)
        {
            newValue = false;
        }
